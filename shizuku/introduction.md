...
Each of the "Execute" means a new process creation, su internally uses sockets to interact with the su daemon, and a lot of time and performance are consumed in such prot cess. (Some poorly designed app will even execute `su` **every time** for each command)
